...
function filterFundedOnly() {
    ...
    let morethanGoal = GAMES_JSON.filter((game) => {
        return game.pledged > game.goal;
        });
...
const display = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} ${totalGames === 1 ? "game" : "games"}. 
Currently, ${totalUnfunded} ${totalUnfunded === 1 ? "game remains" : "games remain"} unfunded. 
We need your help to fund ${totalUnfunded === 1 ? "this" : "these"} ${totalUnfunded === 1 ? "game" : "games"}!`;

// create a new DOM element containing the template string and append it to the description container
const summaryParagraph = document.createElement('p');
summaryParagraph.textContent = display;
